"""
================================================================
COD PYTHON — Cazul a) N=3, M=2, Reteaua PARALEL-SERIALA (PS)
n = 644,000 | R = 5 rulari independente | K = 9
================================================================
Rulare: python3 caz_a_PS.py
================================================================
"""
...
def gnpa1_numpy(n): return np.random.randint(0, 10, size=n)
def gnpa2_secrets(n): return np.array([sec_module.randbelow(10) for _ in range(n)])
def gnpa3_pi(n, offset=0): return pi_digits(n, offset)

# ── Model PS: V = max(min(X_ij)) ─────────────────────────────

def pmf_ps(k, N=N, M=M, K=K_TRUE):
    qk  = (K-k)/(K+1)
    qk0 = (K-k+1)/(K+1)
    if k == 0:
        return (1 - qk**N)**M
...
def F_ps(k, N=N, M=M, K=K_TRUE):
    return (1 - ((K-k)/(K+1))**N)**M

def R_ps(k, N=N, M=M, K=K_TRUE):
    return 1 - F_ps(k, N, M, K)

# ── Simulare PS ───────────────────────────────────────────────

def simulate_PS(gnpa_func, n_sim, pi_offset=0):
    units_total = N * M
    ...
    return matrix.min(axis=2).max(axis=1).astype(np.int32)
...
def mle_complete(data, K_max=30):
    ...
    for K_try in range(K_min, K_max+1):
        ...
        for val, cnt in zip(*np.unique(data, return_counts=True)):
            p = pmf_ps(int(val), K=K_try)
            if p <= 0: ll = -np.inf; break
            ll += cnt * np.log(p)
        if ll > best_ll:
...
def prob_interval_ps(a, b, K_try):
    F_b = F_ps(b-1, K=K_try) if b is not None else 1.0
    F_a = F_ps(a-1, K=K_try) if a > 0 else 0.0
    return F_b - F_a
...
# Scheme censurate PS
INT_C3 = [(0, 3), (3, 6), (6, None)]
INT_C5 = [(0, 3), (3, 5), (5, 7), (7, 8), (8, None)]
...
print("=" * 70)
print(f"CAZ a) N={N}, M={M}, K={K_TRUE} — Reteaua PARALEL-SERIALA (PS)")
print(f"n = {n:,}  |  R = {R} rulari independente")
print("=" * 70)

E_V = sum(k * pmf_ps(k) for k in range(K_TRUE+1))
D_V = sum((k-E_V)**2 * pmf_ps(k) for k in range(K_TRUE+1))
print(f"\nValori teoretice: E[V] = {E_V:.6f}  D[V] = {D_V:.6f}")
print(f"h_ps(5) = {pmf_ps(5)/R_ps(4):.6f}\n")
...
for gnpa_name, gnpa_func in GENERATORS:
    ...
    print(f"{'─'*70}")
    K_hats_c = []; K_hats_c3 = []; K_hats_c5 = []; means_sim = []
    t0 = time.time()
    for r in range(R):
        print(f"  Rularea {r+1}/{R}...", end=" ", flush=True)
        t1 = time.time()
        pi_off = r * n * N * M + 50_000_000  # offset diferit fata de SP
        data = simulate_PS(gnpa_func, n, pi_offset=pi_off)
        means_sim.append(np.mean(data))
        K_c  = mle_complete(data)
        K_c3 = mle_censored(data, INT_C3)
        K_c5 = mle_censored(data, INT_C5)
        K_hats_c.append(K_c); K_hats_c3.append(K_c3); K_hats_c5.append(K_c5)
        print(f"K_c={K_c}, K_c3={K_c3}, K_c5={K_c5} [{time.time()-t1:.1f}s]")
    
    K_hats_c  = np.array(K_hats_c)
    K_hats_c3 = np.array(K_hats_c3)
    K_hats_c5 = np.array(K_hats_c5)
    
    print(f"\n  Media simulata: {np.mean(means_sim):.6f}  (teoretic: {E_V:.6f})")
...
print(f"\n{'='*70}")
print("TABEL COMPARATIV FINAL — Cazul a) PS, N=3, M=2")
print(f"{'='*70}")
