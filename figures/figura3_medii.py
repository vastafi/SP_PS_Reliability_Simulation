"""
Figure 3 — Simulated means vs theoretical values with CLT error bounds.
"""
...
GNPA_ORDER  = ["Java ThreadLocalRandom", "Python secrets", "Digits of π"]
...
for gnpa in GNPA_ORDER:
    ...
    print(f"  Available digits: {len(digits):,}")
    ...
    ok_sp = "OK" if abs(means_sp[gnpa] - mu_sp) <= eps_sp else "OUT"
    ok_ps = "OK" if abs(means_ps[gnpa] - mu_ps) <= eps_ps else "OUT"
...
for ax, sim_vals, mu_t, eps_t, title, unit, ypad in [
    ...
    ax.axhline(y=mu_t, color='red', lw=2.0, ls='-',
    ...
    ax.set_ylabel('Simulated mean', fontsize=11)
...
plt.tight_layout()
out = FIGURES + "figure3_means.png"
plt.savefig(out, dpi=300, bbox_inches='tight', facecolor='white')
plt.close()

print("figure3_means.png saved")
